...
public class KeyValueDB extends SQLiteOpenHelper {
	...
	public final String ACCEPTED = "acceptedvalue";
	//
	...
	public Boolean insertKeyValue(String key, String value, String accepted) {
		SQLiteDatabase db = this.getWritableDatabase();
...
//	}
	...
	public boolean updateValueByKey(String key, String value, String accepted) {
		SQLiteDatabase db = this.getWritableDatabase();
		ContentValues cv = new ContentValues();
		cv.put(KEY, key);
		cv.put(VALUE, value);
		...
		if (nr == 0) {
			insertKeyValue(key, value, accepted);
		}
		return true;
	}
